...
module PuppetBind
  #
  module Provider
    def self.nsupdate_provider(type); end
    ...
    module NsUpdate
      ...
      def maybe_unquote(type, datum)
        quoted_type?(type) ? datum.gsub(%r{^\"(.*)\"$}, '\1') : datum
      end

      def maybe_unescape(type, datum)
        escaped_type?(type) ? datum.gsub(%r{\\;}, ';') : datum
      end
      ...
      def maybe_unspace(type, datum)
        if spaced_type?(type)
          case type
          when 'DS', 'TLSA'
            datum.gsub(%r{^(\d+)\s+(\d+)\s+(\d+)\s+(\w+)\s+(\w+)$}, '\1 \2 \3 \4\5')
          when 'SSHFP'
            datum.gsub(%r{^(\d+)\s+(\d+)\s+(\w+)\s+(\w+)$}, '\1 \2 \3\4')
          end
        else
          datum
        end
      ...
      def query
        unless @query
          dig_text = if keyed?
                       dig("@#{server}", '+noall', '+nosearch', '+norecurse', "+#{query_section}", name, type, '-c', rrclass, '-y', tsig_param)
                     else
                       dig("@#{server}", '+noall', '+nosearch', '+norecurse', "+#{query_section}", name, type, '-c', rrclass)
                     end
          @query = dig_text.lines.map { |line|
            linearray = line.chomp.split(%r{\s+}, 5)
